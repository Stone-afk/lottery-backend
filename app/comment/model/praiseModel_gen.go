...
var (
	...
	praiseRowsExpectAutoSet   = strings.Join(stringx.Remove(praiseFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
func (m *defaultPraiseModel) Insert(ctx context.Context, data *Praise) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserId, data.CommentId, data.DeleteTime, data.DelState)
	}, commentPraiseCommentIdKey, commentPraiseIdKey)
	return ret, err
}
...
func (m *defaultPraiseModel) TransInsert(ctx context.Context, session sqlx.Session, data *Praise) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?)", m.table, praiseRowsExpectAutoSet)
		return session.ExecCtx(ctx, query, data.UserId, data.CommentId, data.DeleteTime, data.DelState)
	}, commentPraiseCommentIdKey, commentPraiseIdKey)
	return ret, err
}
