...
func (s *PeopleLotteryStrategy) CheckLottery(lotteries []*model.Lottery) ([]*model.Lottery, error) {
	var CheckedLotterys []*model.Lottery
	// 筛选满足条件的抽奖
...
// asynq 的实现原理是通过消息队列来实现的，它使用了Redis作为消息队列，通过Redis的list数据结构来存储消息，通过Redis的pub/sub功能来实现消息的发布和订阅，通过Redis的zset数据结构来实现消息的延迟和定时。
// asynq 的应用场景有很多，比如可以用来处理异步任务、定时任务、延迟任务、重试任务等，适用于各种异步任务处理场景。
// asynq 的实现方式是通过Redis的list数据结构来存储消息，通过Redis的pub/sub功能来实现消息的发布和订阅，通过Redis的zset数据结构来实现消息的延迟和定时。
// asynq 的使用有一些注意事项，比如需要保证消息的幂等性，需要保证消息的可靠性，需要保证消息的顺序性，需要保证消息的一致性等。
