...
func (m *defaultUserModel) SetAdmin(ctx context.Context, uid int64) error {
	usercentUserIdKey := fmt.Sprintf("%s%v", cacheLooklookUsercenterUserIdPrefix, uid)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set is_admin = CASE WHEN is_admin = 0 THEN 1 ELSE 0 END where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, uid)
	}, usercentUserIdKey)
	return err
}

func (m *defaultUserModel) FindUserInfoByUserIds(ctx context.Context, userIds []int64) ([]*pb.UserInfoForComment, error) {
	...
	query := fmt.Sprintf("select id,nickname,avatar from %s where id in (%s)", m.table, commaSeparated)
